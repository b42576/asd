    ...
    $('#getStoresJSON').on('click', function () {
		...
		$.ajax({
			...
			error: function (jqX, stat, err) {
				console.log(jqX);
				$('#store').empty();
				var whatsreturned = 'jsonp('+jqX['responseText']+')';
				var objJSON = $.parseJSON(whatsreturned);
				console.log(objJSON);
				i=0;
				j=objJSON.length;
				//console.log(j);
				for (i; i<j; i++){
					console.log(whatsreturned.stores[i]);
					var storeinfo = whatsreturned.stores[i];
					$('#store').append('<option value="'+storeinfo.store+'">'+storeinfo.store+'</option>');
				}
			}
		});
		return false;
